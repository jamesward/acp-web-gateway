...
fun statusTimerHtml(activity: String, elapsed: String, usage: String? = null): String = createHTML(prettyPrint = false).div(classes = Css.STATUS_WRAP) {
    ...
    div(classes = Css.STATUS_TEXT) {
        id = Id.TASK_STATUS
        val parts = mutableListOf("$activity \u00b7 $elapsed")
...
fun toolBlockHtml(
    ...
    div(classes = Css.TOOL_BLOCK) {
        ...
        val activeTools = entries.filter { it.status != "completed" && it.status != "failed" }
            ...
            summary(classes = Css.TOOL_HEADER) {
                div(classes = Css.TOOL_HEADER_LEFT) {
                    ...
                    span(classes = Css.TOOL_SUMMARY) {
                        ...
                        if (activeTools.isNotEmpty()) parts.add("${activeTools.size} running")
            ...
            div(classes = Css.TOOL_LIST) {
                for (entry in entries) {
                    div(classes = Css.TOOL_ROW) {
                        ...
                        if (entry.content != null || entry.contentHtml != null) {
                            details {
...
private fun toolKindIcon(kind: String?): String = when (kind) {
    "read" -> "\uD83D\uDCC4"    // page
    "edit" -> "\u270F\uFE0F"     // pencil
    "delete" -> "\uD83D\uDDD1\uFE0F" // wastebasket
    "move" -> "\u2194\uFE0F"     // left-right arrow
...
private fun FlowContent.toolEntryIcon(status: String, kind: String? = null) {
    if (status != "completed" && status != "failed" && kind != null) {
        val icon = toolKindIcon(kind)
        if (icon != "null") {
            span(classes = "${Css.TOOL_ICON} ${Css.TOOL_ICON_RUNNING} ${Css.PULSE}") { +icon }
            return
        }
    }
    val (iconText, iconClass) = when (status) {
        "completed" -> "\u2713" to "${Css.TOOL_ICON} ${Css.TOOL_ICON_DONE}"
        "failed" -> "\u2717" to "${Css.TOOL_ICON} ${Css.TOOL_ICON_FAIL}"
...
private fun FlowContent.toolEntryTitle(entry: ToolCallDisplay) {
    val titleClass = when (entry.status) {
        "completed" -> "${Css.TOOL_TITLE} ${Css.TOOL_TITLE_DONE} ${Css.TRUNCATE}"
        "failed" -> "${Css.TOOL_TITLE} ${Css.TOOL_TITLE_FAIL} ${Css.TRUNCATE}"
...
fun permissionContentHtml(
    ...
    div(classes = Css.PERM_BUTTONS) {
        for (opt in options) {
            val btnClass = if (opt.kind.startsWith("allow")) Css.PERM_BTN_ALLOW else Css.PERM_BTN_DENY
